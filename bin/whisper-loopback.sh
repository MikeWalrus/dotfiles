...
fmt=wav # the audio format extension of the stream (TODO: auto detect)
step_s=10
model="small.en"
...
# ffmpeg -loglevel quiet -y -re -probesize 32 -f pulse -i alsa_output.pci-0000_00_1f.3-platform-skl_hda_dsp_generic.HiFi__hw_sofhdadsp__sink -c copy /tmp/whisper-live0.${fmt} &
pw-record --rate 16000 --target 3124 /tmp/whisper-live0.${fmt} &
if [ $? -ne 0 ]; then
...
while [ $running -eq 1 ]; do
    ...
    whisper.cpp -t 8 -m /usr/share/whisper.cpp-model-small.en/small.en.bin -f /tmp/whisper-live.wav --no-timestamps -otxt 2> /tmp/whispererr | tail -n 1
